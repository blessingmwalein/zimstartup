...
export default function VCCPage() {
  ...
  const filteredVCCs = vccList.filter((vcc) => {
    const matchesSearch = vcc.vcc_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         vcc.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         vcc.competition_category?.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesStatus = statusFilter === "ALL" || vcc.status === statusFilter;
        ...
        <div className="container mx-auto px-4 md:px-8">
          ...
          <div className="mb-6 flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search VCC by name or category..."
                value={searchQuery}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {vccList.filter(v => v.status === "OPEN").length}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-600">
                    {vccList.filter(v => v.status === "CLOSED").length}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-yellow-600">
                    {vccList.filter(v => v.status === "PENDING" || v.status === "DRAFT").length}
                  </p>
                </div>
              ...
              {filteredVCCs.map((vcc) => (
                ...
                >
                  ...
                  <div className="relative h-40 bg-gradient-to-br from-blue-600 to-purple-600 overflow-hidden">
                    {vcc.featured_image_url ? (
                      <img
                        src={vcc.featured_image_url}
                        alt={vcc.vcc_name}
                  ...
                  <div className="p-5">
                    <div className="mb-3">
                      ...
                      <p className="text-xs text-gray-500 font-medium">
                        {vcc.competition_category}
                      </p>
                    </div>

                    <p className="text-sm text-gray-600 line-clamp-2 mb-4">
                      {vcc.short_description || "No description available"}
                    </p>
